...
router.get('/download/tiktok', async (req, res) => {
  try {
    ...
    // 📥 Request ke API autoresbot
    const apiUrl = `https://api.autoresbot.com/api/downloader/tiktok?apikey=NICKYSTORE&url=${encodeURIComponent(url)}`;
    const { data } = await axios.get(apiUrl);

    if (!data || !data.status) {
      return res.status(500).json({
        status: false,
        message: "Gagal download dari TikTok (API autoresbot error)"
      });
    }
    // ✅ Response ke user
    res.json({
      ...
      creator,
      result: {
        video_nowm: data.data.url,
        audio_url: data.data.audio || null,
